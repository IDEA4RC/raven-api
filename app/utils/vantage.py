...
def create_vantage_client(username: str, password: str) -> Optional[Any]:
    try:
...
def create_vantage_organization(organization_data: Dict[str, Any]) -> Optional[dict]:
...
def create_vantage_user(user_data: Dict[str, Any]) -> Optional[dict]:
    # Client will be imported in create_vantage_client
    client = create_vantage_client("root", "root")
    if not client:
        return None
...
def algorithm_org_input(collaboration_data: Dict[str, Any]) -> Optional[dict]:
    # Client will be imported in create_vantage_client
    # client = create_vantage_client(        "root",        "root"    )
    ...
    payload = {
        ...
        "databases": [
            [{"type": "dataframe", "dataframe_id": df_id} for df_id in DATAFRAME_IDS]
        ],
