...
@router.get(
    "/cohort/{cohort_id}", response_model=List[schemas.cohort_result.CohortResult]
)
...
def count_cohort_results(
    *,
    ...
    """
    count = cohort_result_service.count_results_for_cohort(db=db, cohort_id=cohort_id)
    return count


@router.patch(
    "/cohort/{cohort_id}/data-id", response_model=schemas.cohort_result.CohortResult
)
    ...
    *,
    ...
    try:
        cohort_result = cohort_result_service.update_cohort_result(
            db=db, cohort_id=cohort_id, data_id=current_data_id, obj_in=cohort_result_in
        )
        return cohort_result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "message": f"Deleted {deleted_count} cohort results",
        }
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
@router.post(
    "/cohort/{cohort_id}/bulk", response_model=List[schemas.cohort_result.CohortResult]
)
