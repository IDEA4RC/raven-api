...
class PermitService(BaseService[Permit, PermitCreate, PermitUpdate]):
    ...
    def create_with_history(
        self, db: Session, *, obj_in: PermitCreate, user_id: int
    ) -> Permit:
        """
    ...
    def update_permit_status(
        ...
        *,
        ...
        permit_update = PermitUpdate(
            status=status, update_date=datetime.now(timezone.utc)
        )
        ...
        if status == PermitStatus.SUBMITTED:
            ...
            action = "Rejected data access application"
    ...
    def update_with_history(
        ...
        *,
        ...
        if (
            obj_in.permit_name is not None
            and updated_permit.permit_name != old_permit_name
        ):
            changes.append(f"permit name")
        if (
            obj_in.expiration_date is not None
            and updated_permit.expiration_date != old_expiration_date
        ):
        ...
        if (
            obj_in.coes_granted is not None
            and updated_permit.coes_granted != old_coes_granted
        ):
